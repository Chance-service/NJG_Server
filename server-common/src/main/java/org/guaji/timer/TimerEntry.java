 ...
 * @change xulinqs
 */
public class TimerEntry implements Comparable<TimerEntry> {
	/**
	 * 时间配置, 秒
	 */
	 ...
	 * 事件监听对象
	 */
	private transient ITimerListener listener;
	 ...
	 */
	public TimerEntry(String name, Date date, ITimerListener listener) throws MyException {
		setName(name);
		this.listener = listener;
		Calendar alarm = GuaJiTime.getCalendar();
		alarm.setTime(date);
	 ...
	 * @exception MyException
	 */
	public TimerEntry(String name, int delaySecond, boolean isRepeating, ITimerListener listener) throws MyException {
		if (delaySecond <= 0) {
			throw new MyException("dealy second less than 1.");
		}
		setName(name);
	 ...
	 * 定时器比较接口
	 */
	public int compareTo(TimerEntry object) {
		TimerEntry entry = (TimerEntry) object;
	...
	public boolean equals(Object object) {
		if (object == null || !(object instanceof TimerEntry)) {
			return false;
		}
		TimerEntry entry = (TimerEntry) object;
	 ...
	 */
	public TimerEntry(String name, int second, int minute, int hour, int dayOfMonth, int dayOfWeek, ITimerListener listener) throws MyException {
	 ...
	 *                if the alarm date is in the past (or less than 1 second away from the current date).
	 */
	public TimerEntry(String name, int[] seconds, int[] minutes, int[] hours, int[] daysOfMonth, int[] daysOfWeek, ITimerListener listener) throws MyException {
		setName(name);
	...
	public void updateAlarmTime() {
		Calendar now = GuaJiTime.getCalendar();
		if (isRelative) {
