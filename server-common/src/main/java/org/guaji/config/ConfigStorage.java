...
import org.guaji.config.ConfigBase.Id;
import org.guaji.cryption.Md5;
import org.guaji.log.Log;
import org.guaji.os.MyException;
import org.guaji.os.OSOperator;
import org.guaji.util.GuaJiJsonUtil;
...
public class ConfigStorage {
	 ...
	 * 配置对象列表
	 */
	private List<ConfigBase> configList = null;
	/**
	 * 配置对象映射表
	 */
	private Map<Object, ConfigBase> configMap = null;
	/**
	 * 域映射
	 */
	 ...
	 * @throws Exception
	 */
	public ConfigStorage(Class<?> cfgClass) throws Exception {
		this.cfgClass = cfgClass;

		if (cfgClass.getAnnotation(ConfigManager.XmlResource.class) != null) {
			ConfigManager.XmlResource xmlRes = cfgClass.getAnnotation(ConfigManager.XmlResource.class);

			filePath = App.getInstance().getWorkPath() + xmlRes.file();
			Log.logPrintln("load config: " + filePath);

			if ("map".equals(xmlRes.struct())) {
				configMap = new TreeMap<Object, ConfigBase>();
			...
			loadXmlConfig();
		} else if (cfgClass.getAnnotation(ConfigManager.KVResource.class) != null) {
			ConfigManager.KVResource kvRes = cfgClass.getAnnotation(ConfigManager.KVResource.class);

			configList = new LinkedList<ConfigBase>();
			filePath = App.getInstance().getWorkPath() + kvRes.file();
			Log.logPrintln("load config: " + filePath);

			loadKVConfig();
		} else if (cfgClass.getAnnotation(ConfigManager.JsonResource.class) != null) {
			ConfigManager.JsonResource jsonRes = cfgClass.getAnnotation(ConfigManager.JsonResource.class);

			filePath = App.getInstance().getWorkPath() + jsonRes.file();
			Log.logPrintln("load config: " + filePath);
	 ...
	 * @throws Exception
	 */
	public ConfigStorage(Class<?> cfgClass,String dir) throws Exception {
		this.cfgClass = cfgClass;

		if (cfgClass.getAnnotation(ConfigManager.XmlResource.class) != null) {
			ConfigManager.XmlResource xmlRes = cfgClass.getAnnotation(ConfigManager.XmlResource.class);

			filePath = App.getInstance().getWorkPath() +dir+"/"+ xmlRes.file();
			Log.logPrintln("load config: " + filePath);
			...
			configList = new LinkedList<ConfigBase>();
			filePath = App.getInstance().getWorkPath() +dir+"/"+kvRes.file();
			Log.logPrintln("load config: " + filePath);
			...
			filePath = App.getInstance().getWorkPath() +dir+"/"+ jsonRes.file();
			Log.logPrintln("load config: " + filePath);
	...
	public void assemble() throws Exception{
		if(configList!=null){
			for(ConfigBase configBase:configList){
				assembleConfig(configBase, configBase.getKey());
			}
		}else{
			Iterator<Object> iterator=configMap.keySet().iterator();
			while(iterator.hasNext()){
				Object key=iterator.next();
				ConfigBase configBase=configMap.get(key);
				assembleConfig(configBase, key);
			}
		}
	}
	
	private void assembleConfig(ConfigBase configBase,Object key) throws Exception{
		if (!configBase.assemble()) {
			throw new Exception("config assemble failed: " + cfgClass.getName() + ", key: " + key);
		}
	}
	...
	private void loadXmlConfig() throws Exception {
		try {
			...
			if (!file.exists()) {
				throw new MyException("config file not exist: " + filePath);
			}
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				if (attrMap != null) {
					...
					if (className != null && !"".equals(className)) {
						...
						if (cfg instanceof ConfigBase) {
							ConfigBase configBase = (ConfigBase) cfg;
							...
							for (int j = 0; j < attrMap.getLength(); j++) {
								...
								if (field == null) {
									throw new MyException("config class field not exist, file: " + filePath + ", field: " + attrNode.getNodeName().trim());
								}
								...
								if (id != null) {
									// id标注重复
									if (cfgKey != null) {
										throw new MyException("config id annotation duplicate: " + filePath + ", nodeName: "+ attrNode.getNodeName());
									}
	...
	private void loadJsonConfig() throws Exception {
		try {
			...
			String fileData = OSOperator.readTextFile(filePath);
			if (fileData != null) {
				...
				if (configList != null && jsonObject.containsKey("list")) {
	            	...
	            	throw new MyException("json root key must be list or map");
	            }
					...
					if (cfg instanceof ConfigBase) {
			            ...
			            while (it.hasNext()) {  
							...
							if (id != null) {
								// id标注重复
								if (cfgKey != null) {
									throw new MyException("config id annotation duplicate: " + filePath);
								}
								String type = field.getType().toString();
								if (type.indexOf("int") >= 0 || type.indexOf("Integer") >= 0) {
	...
	private Field getField(Object instance, String attrName) {
		...
		if (field == null) {
			field = OSOperator.getClassField(instance, attrName);
			if (field != null) {
	...
	public void setAttr(Object instance, String attrName, String attrValue) throws Exception {
		...
		if (field == null) {
			Log.errPrintln(String.format("config class cannot find field, class: %s, field: %s", instance.getClass().getName(), attrName));
		...
		try {
			...
			if (setMethod != null) {
				if (field.getType() == int[].class){
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue,new TypeToken<int[]>() {}.getType()));
				} else if (field.getType() == Integer[].class){
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue,new TypeToken<Integer[]>() {}.getType()));
				} else if (field.getType() == float[].class){
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue,new TypeToken<float[]>() {}.getType()));
				} else if (field.getType() == Float[].class){
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue,new TypeToken<Float[]>() {}.getType()));
				} else if (field.getType() == double[].class){
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue,new TypeToken<double[]>() {}.getType()));
				} else if (field.getType() == Double[].class){
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue,new TypeToken<Double[]>() {}.getType()));
				}else if (field.getType() == String[].class){
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue,new TypeToken<String[]>() {}.getType()));
				} else if (field.getType() == Map.class){
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue,new TypeToken<Map<String, String>>() {}.getType()));
				...
				if (field.getType() == int[].class) {
					...
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue, new TypeToken<long[]>() {}.getType()));
				} else if (field.getType() == Long[].class) {
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue, new TypeToken<Long[]>() {}.getType()));
				} else if (field.getType() == float[].class) {
					field.set(instance, GuaJiJsonUtil.getJsonInstance().fromJson(attrValue, new TypeToken<float[]>() {}.getType()));
			...
			Log.errPrintln(String.format("config setAttr failed, class: %s, field: %s", instance.getClass().getName(), attrName));
			throw e;
		}
	}
	...
	public boolean checkValid() {
		...
		if (configMap != null) {
			for (Map.Entry<Object, ConfigBase> entry : configMap.entrySet()) {
	 ...
	 * @return
	 */
	public List<? extends ConfigBase> getConfigList() {
		return configList;
	}
	 ...
	 * @return
	 */
	public Map<Object, ? extends ConfigBase> getConfigMap() {
		return configMap;
	}
}
