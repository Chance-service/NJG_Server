...
public class OSOperator {
	...
	public static String randomString(int length) {
		...
		try {
			for (int i = 0; i < length; i++) {
				char ch = 0;
				if (GuaJiRand.randInt(0, 1) == 0) {
					ch = (char) GuaJiRand.randInt('a', 'z');
				} else {
					ch = (char) GuaJiRand.randInt('A', 'Z');
				}
				builder.append(ch);
	...
	public static String randomString(int length, String charSequence) {
		...
		for (int i = 0; i < length; i++) {     
		    try {
		    	int index = GuaJiRand.randInt(0, charSequence.length()-1);
	...
	public static void printOsEnv() {
		...
		Log.logPrintln("UserDir: " + userDir);

		// 系统路径
		String homeDir = System.getProperty("java.home");
		Log.logPrintln("JavaHome: " + homeDir);
	}
