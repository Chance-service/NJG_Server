package org.guaji.net.websocket;
...
public class WebSocketUtil {
	 ...
	 * @return
	 */
	public static boolean responseWebSockeHandShake(GuaJiSession session, IoBuffer inBuffer) {
        try{
            ...
            session.sendMessage(new HandShakeResponse(response));
            return true;
        } catch(Exception e){
        	session.close(true);
        	MyException.catchException(e);
	 ...
	 * @return
	 */
	public static IoBuffer decodeWebSocketDataBuffer(IoBuffer inBuffer, GuaJiSession session) {
		...
		do {
			...
			if (opCode == FRAME_BINARY) {
				session.setWebSessionMode(GuaJiSession.BINARY_WEB_SESSION);
			} else if (opCode == FRAME_TEXT) {
				session.setWebSessionMode(GuaJiSession.JSON_WEB_SESSION);
			}
