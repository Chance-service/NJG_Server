...
public class MysqlConnection {
	...
	protected MysqlConnection() {
		lastHeartBeat = GuaJiTime.getMillisecond();
	}
	...
	protected synchronized boolean checkConnection() {
		try {
			if (connection == null || connection.isClosed()) {
				if (connection != null) {
					Log.logPrintln(String.format("mysql reconnect, dbHost: %s, dbUser: %s, dbPwd: %s", dbHost, dbUser, dbPwd));
				}
	...
	protected void keepAlive() {
		try {
			// mysql 连接保持活跃(60s)
			if (defaultStatement != null && GuaJiTime.getMillisecond() - lastHeartBeat >= 60000) {
				// 重连
				checkConnection();
