...
import org.guaji.util.GuaJiByteOrder;
...
public class GuaJiZmq {
	 ...
	 * @throws MyException
	 */
	public boolean init(ZMQ.Context context, int type) throws MyException {
	...
	public OctetsStream checkStream() {
		if (stream == null) {
			stream = OctetsStream.create(HZMQ_BUFFER_SIZE, true);
		}
		stream.clear();
		return stream;
	}
	 ...
	 * @return
	 */
	public boolean sendProtocol(Protocol protocol, int flag) {
		if (protocol != null) {
			checkStream();
			try {
	 ...
	 * @return
	 */
	public Protocol recvProtocol(int flag) {
		checkStream();
		...
		if (socket != null) {
			try {
				...
				if (socket.monitor(addr, events)) {
					...
					// 创建监视器, 连接到对端
					monitor = GuaJiZmqManager.getInstance().context.socket(ZMQ.PAIR);
					if (monitor != null) {
						monitor.setLinger(0);
		...
		if (socket != null && monitor != null) {
			...
			try {
				...
				if (eventCnt > 0 && monitorPoll[0].isReadable()) {
					...
					Short events = 0;
					if (recvSize > 0) {
						events = GuaJiByteOrder.reverseShort(stream.readShort());
					}
