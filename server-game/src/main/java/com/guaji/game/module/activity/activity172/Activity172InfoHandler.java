...
* 类说明
*/
public class Activity172InfoHandler implements IProtocolHandler{
	@Override
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY172_PICKUP_INFO_S, builder));
		return true;
	}
	...
	public static ActivityCallInfo.Builder generateInfo(Player player) {
		...
		ActivityTimeCfg activityTimeCfg = ActivityUtil.getCurActivityTimeCfg(activityId);
		Activity172Status status = ActivityUtil.getActivityStatus(player.getPlayerData(), activityId, activityTimeCfg.getStageId(),
				Activity172Status.class);
