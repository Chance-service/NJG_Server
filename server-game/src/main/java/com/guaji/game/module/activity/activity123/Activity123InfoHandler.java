...
* 类说明
*/
public class Activity123InfoHandler implements IProtocolHandler{
	@Override
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		ReleaseURTimesCfg123 timesCfg = ReleaseURTimesCfg123.getTimesCfgByVipLevel(0);
		if (timesCfg == null) {
			player.sendError(protocol.getType(), Status.error.CONFIG_NOT_FOUND);
			return true;
		}

		Activity123Info.Builder builder = generateInfo(player);
		
		player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY123_UR_INFO_S_VALUE, builder));
		return true;
	}
	...
	public static Activity123Info.Builder generateInfo(Player player) {
		int activityId = Const.ActivityId.ACTIVITY123_UR_VALUE;
		long currentTime = System.currentTimeMillis();
		ReleaseURTimesCfg123 timesCfg = ReleaseURTimesCfg123.getTimesCfgByVipLevel(player.getVipLevel());
		ActivityTimeCfg activityTimeCfg = ActivityUtil.getCurActivityTimeCfg(activityId);
		Activity123Status status = ActivityUtil.getActivityStatus(player.getPlayerData(), activityId, activityTimeCfg.getStageId(),
				Activity123Status.class);
