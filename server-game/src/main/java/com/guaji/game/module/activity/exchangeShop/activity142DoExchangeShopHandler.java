...
import com.guaji.game.config.ExchangeShop142Cfg;
 ...
 * 兑换所
 */
public class activity142DoExchangeShopHandler implements IProtocolHandler {
    @Override
    public boolean onProtocol(AppObj appObj, Protocol protocol) {

        Player player = (Player) appObj;
        // 检测活动是否开放
        int activityId = Const.ActivityId.ACTIVITY142_EXCHANGE_SHOP_VALUE;
        ...
        ExchangeShop142Cfg exchangeShop142Cfg = ConfigManager.getInstance().getConfigByKey(ExchangeShop142Cfg.class, exchangeId);

        List<ItemInfo> consumeInfo = new ArrayList<ItemInfo>();
        //根据兑换次数修正兑换消耗数目
        for (ItemInfo item : exchangeShop142Cfg.getExchangeConsumeInfo()) {
        ...
        if (exchangeShop142Cfg.getIsReset()) {
        ...
        //有限制
        if (exchangeShop142Cfg.getMaxExchangeTimes() != 0) {
            if (times + alreadyTimes > exchangeShop142Cfg.getMaxExchangeTimes()) {
        ...
        //根据兑换次数修正兑换消耗数目
        for (Item item : exchangeShop142Cfg.getExchangeAwardInfo().getAwardItems()) {
        ...
        if (exchangeShop142Cfg.getIsReset()) {
            ...
            player.getPlayerData().updateActivity(Const.ActivityId.ACTIVITY142_EXCHANGE_SHOP_VALUE, timeCfg.getStageId(), true);
        } else {
        ...
        //发送服务器礼包协议
        player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY142_EXCHANGE_SHOP_INFO_S_VALUE, BuilderUtil.genExchangeShopStatus(lastTime, exchangeShopStatus,stateEntity.getExchangeMap())));
        ...
        player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY142_DO_EXCHANGE_SHOP_S, builder));
        return true;
    }
}
