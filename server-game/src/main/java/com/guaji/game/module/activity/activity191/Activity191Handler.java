...
import com.guaji.game.config.ActivityDailyQuestPointCfg;
import com.guaji.game.config.ActivityQuestCfg;
 ...
 * 循環關卡協定
 */
public class Activity191Handler implements IProtocolHandler {
	...
	protected boolean onTakeQuestAward(Protocol protocol,ActivityTimeCfg timeConfig,Player player,int action,Activity191Status status) {
		CycleStageReq request = protocol.parseProtocol(CycleStageReq.getDefaultInstance());
		
		int id = request.getChoose();
		
		ActivityQuestCfg questCfg = ConfigManager.getInstance().getConfigByKey(ActivityQuestCfg.class, id);
	 ...
	 * @param status
	 */
	private static void SyncInfo(ActivityTimeCfg timeConfig,int action,Player player, Activity191Status status) {
		CycleStageResp.Builder builder = getBuilder(timeConfig,action,player,status);
		player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY191_CYCLE_STAGE_S, builder));
	}
	
	private static CycleStageResp.Builder getBuilder(ActivityTimeCfg timeConfig,int action,Player player,Activity191Status status) {
		// 返回包
		...
		if ((action == cycleStage_questInfo) || (action == cycleStage_take_quest_award)) {
			...
			for (Map.Entry<Integer, DailyQuestItem> entry : map.entrySet()) {
				...
				ActivityDailyQuestCfg dailyQuestCfg = ConfigManager.getInstance().getConfigByKey(ActivityDailyQuestCfg.class,
	...
	private static void ChkDailyQuest(ActivityTimeCfg timeConfig,Player player,Activity191Status status) {
		if (status == null) {
			return;
		}
		int count=0;
		for (ActivityDailyQuestCfg dailyQuestCfg : ConfigManager.getInstance().getConfigMap(ActivityDailyQuestCfg.class).values()) {
	...
	private void onTakeDailyPointAward(Protocol protocol,ActivityTimeCfg timeConfig,Player player,int action,Activity191Status status) {
				
		CycleStageReq request = protocol.parseProtocol(CycleStageReq.getDefaultInstance());

		int count = request.getChoose();

		ActivityDailyQuestPointCfg dailyQuestPointCfg = ConfigManager.getInstance().getConfigByKey(ActivityDailyQuestPointCfg.class,
				count);
	...
	protected boolean onTakeDailyAward(Protocol protocol,ActivityTimeCfg timeConfig,Player player,int action,Activity191Status status) {
