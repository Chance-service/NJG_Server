...
* 类说明
*/
public class Activity173Handler implements IProtocolHandler{
	...
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		// 检测活动是否开放
		int activityId = Const.ActivityId.ACTIVITY173_NEWROLE_VALUE;
		...
		Activity173Status status = ActivityUtil.getActivityStatus(player.getPlayerData(), activityId, timeCfg.getStageId(), Activity173Status.class);
		// 使用免费抽奖的时间
...
//		long currentTime = System.currentTimeMillis();
		...
		if (searchTimes == TIMES_TYPE_TEN) {
			...
			if (isCost && consumeItems.checkConsume(player)) { 
				if(!consumeItems.consumeTakeAffect(player, Action.ACTIVITY173_NEW_ROLE)) {
					payGold = tenCost;
				} 
			} else {
				payGold = tenCost;
...
//			}
	    ...
	    if (payGold > 0) {
			...
			player.consumeGold(payGold, Action.ACTIVITY173_NEW_ROLE);
		...
		for (int i = 1; i <= searchTimes; i++) {
			...
			if  ((SysBasicCfg.getInstance().getNewRoleGuarant()-1) == totalTimes) { //第69次了
			...
			if (((SysBasicCfg.getInstance().getNewRoleGuarant()-1) == totalTimes) ||
...
//			if 	(poolid == ReleaseURDropCfg173.POOL_TYPE_SEARCH) {
...
//						poolid = ReleaseURDropCfg173.POOL_TYPE_BOX;
...
//			}
		...
		awards.rewardTakeAffectAndPush(player, Action.ACTIVITY173_NEW_ROLE, 0);
		
		// 更新玩家的活动数据
		player.getPlayerData().updateActivity(activityId, timeCfg.getStageId());
		
		ActivityCallInfo.Builder builder = Activity173InfoHandler.generateInfo(player);

		for (String itemStr : awardsList) {
			builder.addReward(itemStr);
		}
		
		player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY173_NEWROLE_DRAW_S, builder));
		...
		BehaviorLogger.log4Service(player,Source.USER_OPERATION, Action.ACTIVITY173_NEW_ROLE, Params.valueOf("searchTimes", searchTimes),
