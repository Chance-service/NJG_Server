...
import com.guaji.game.config.ReleaseURDropCfg121;
 ...
 * 新版神将投放
 */
public class ReleaseURHandler121 implements IProtocolHandler {
	...
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		ReleaseURStatus121 status = ActivityUtil.getActivityStatus(player.getPlayerData(), activityId, timeCfg.getStageId(), ReleaseURStatus121.class);
		// 使用免费抽奖的时间
		...
		if (searchTimes == TIMES_TYPE_TEN) {
			...
			// 是否免费
			int freeCD = (int) Math.max(ReleaseURInfoHandler121.convertTimeToMillisecond(timesCfg.getFreeCountDown()) - (currentTime - lastFreeTime), 0);
			if (freeCD == 0) {
				payGold = 0;
				payTimes = 0;
		...
		for (int i = 1; i <= searchTimes; i++) {
			...
			// 掉落物品
			List<ItemInfo> searchItems = ReleaseURDropCfg121.treasureRaiderDropItem(totalTimes + 1);
			...
			awards.rewardTakeAffectAndPush(player, Action.RELEASE_UR3, 0);
		}

		ReleaseURInfo.Builder infoBuilder = ReleaseURInfoHandler121.generateInfo(player);
		...
		builder.setLotteryCost(SysBasicCfg.getInstance().getReleaseUrLotteryCost3());
		...
		player.sendProtocol(Protocol.valueOf(HP.code.RELEASE_UR_DRAW3_S, builder));
		// 更新玩家的活动数据
		player.getPlayerData().updateActivity(activityId, timeCfg.getStageId());
		// BI 日志 (freeTimes 本次寻宝开始前今日剩余的免费次数)
		BehaviorLogger.log4Platform(player, Action.RELEASE_UR3, Params.valueOf("searchTimes", searchTimes),
