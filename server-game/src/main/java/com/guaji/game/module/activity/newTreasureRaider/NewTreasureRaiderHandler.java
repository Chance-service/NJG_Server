package com.guaji.game.module.activity.newTreasureRaider;
 ...
 * 抽卡
 */
public class NewTreasureRaiderHandler implements IProtocolHandler {
	...
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		NewTreasureRaiderTimesCfg timesCfg = NewTreasureRaiderTimesCfg.getTimesCfgByVipLevel(0);
		if (timesCfg == null) {
		...
		if (searchTimes == TIMES_TYPE_TEN) {
			...
			// 是否免费
			int freeCD = (int) Math.max(NewTreasureRaiderInfoHandler.convertTimeToMillisecond(timesCfg.getFreeCountDown())- (currentTime - lastFreeTime), 0);
			if (freeCD == 0) {
				payGold = 0;
				payTimes = 0;
		...
		// 扣除钻石
		if (payGold > 0) {
			player.consumeGold(payGold, Action.NEW_TREASURE_RAIDER);
		...
		for (int i = 1; i <= searchTimes; i++) {
			...
			// 掉落物品
			List<ItemInfo> searchItems = NewTreasureRaiderDropCfg.treasureRaiderDropItem(totalTimes + 1);
			...
			/*-------------处理玩家下次抽奖翻倍-------------*/
			/*
			boolean isActiveMultiple = SysBasicCfg.getInstance().getNewTreasureRaiderLimitTimes()
					.contains(status.getTotalTimes());
			if (!isActiveMultiple) {
				// 从抽取区间配置中判断是否激活
				isActiveMultiple = NewTreasureRaiderMultipCfg.isActiveMultiple(status.getLuckyValue());
			}
			if (isActiveMultiple) {
				int buffId = NewTreasureRaiderMultipBuffCfg.randomItemId();
				NewTreasureRaiderMultipBuffCfg buffCfg = ConfigManager.getInstance()
						.getConfigByKey(NewTreasureRaiderMultipBuffCfg.class, buffId);
				if (buffCfg != null) {
					int nexMultiple = buffCfg.getMultiple();
					int nextOverTime = NewTreasureRaiderInfoHandler.convertTimeToMillisecond(buffCfg.getOverTime());
					...
					Log.logPrintln(String.format("%s: %s=%s", "新夺宝奇兵活动翻倍配置未找到", "buffid", buffId));
				}
			}
			*/
			/*---------奖励物品--------*/
			awards.rewardTakeAffectAndPush(player, Action.NEW_TREASURE_RAIDER, 0);
		}

		HPNewTreasureRaiderInfoSync.Builder infoBuilder = NewTreasureRaiderInfoHandler.generateInfo(player);
		...
		player.sendProtocol(Protocol.valueOf(HP.code.NEW_TREASURE_RAIDER_SEARCH_S_VALUE, builder));
		// 更新玩家的活动数据
		player.getPlayerData().updateActivity(activityId, timeCfg.getStageId());
		// BI 日志 (freeTimes 本次寻宝开始前今日剩余的免费次数)
		BehaviorLogger.log4Platform(player, Action.NEW_TREASURE_RAIDER, Params.valueOf("searchTimes", searchTimes),
