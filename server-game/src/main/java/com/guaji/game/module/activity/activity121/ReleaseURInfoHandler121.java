 ...
 * 神将投放活动信息同步
 */
public class ReleaseURInfoHandler121 implements IProtocolHandler {
	...
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		ReleaseURTimesCfg121 timesCfg = ReleaseURTimesCfg121.getTimesCfgByVipLevel(0);
		if (timesCfg == null) {
		...
		player.sendProtocol(Protocol.valueOf(HP.code.RELEASE_UR_INFO3_S, builder));
		return true;
	}
	...
	public static ReleaseURInfo.Builder generateInfo(Player player) {
		...
		ActivityTimeCfg activityTimeCfg = ActivityUtil.getCurActivityTimeCfg(activityId);
		ReleaseURStatus121 status = ActivityUtil.getActivityStatus(player.getPlayerData(), activityId, activityTimeCfg.getStageId(),
		        ReleaseURStatus121.class);
		...
		int genLeftTimes = ReleaseURDropCfg121.getLimitTimes(totalTimes);
