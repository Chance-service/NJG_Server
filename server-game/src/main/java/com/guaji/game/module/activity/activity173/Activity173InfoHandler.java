...
* 类说明
*/
public class Activity173InfoHandler implements IProtocolHandler{
	@Override
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY173_NEWROLE_INFO_S, builder));
		return true;
	}
	...
	public static ActivityCallInfo.Builder generateInfo(Player player) {
		...
		ActivityTimeCfg activityTimeCfg = ActivityUtil.getCurActivityTimeCfg(activityId);
		Activity173Status status = ActivityUtil.getActivityStatus(player.getPlayerData(), activityId, activityTimeCfg.getStageId(),
				Activity173Status.class);
...
//		}
		...
		int genLeftTimes = SysBasicCfg.getInstance().getNewRoleGuarant();
