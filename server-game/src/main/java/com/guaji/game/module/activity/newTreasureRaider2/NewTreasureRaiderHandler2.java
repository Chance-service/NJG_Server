...
import com.guaji.game.protocol.Activity2.HPNewTreasureRaiderSearch2;
 ...
 * 抽卡
 */
public class NewTreasureRaiderHandler2 implements IProtocolHandler {
	...
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		HPNewTreasureRaiderSearch2 req = protocol.parseProtocol(HPNewTreasureRaiderSearch2.getDefaultInstance());
		...
		if (searchTimes == TIMES_TYPE_TEN) {
			...
			// 是否免费
			int freeCD = (int) Math
					.max(NewTreasureRaiderInfoHandler2.convertTimeToMillisecond(timesCfg.getFreeCountDown())
		...
		// 扣除钻石
		if (payGold > 0) {
			player.consumeGold(payGold, Action.NEW_TREASURE_RAIDER2);
		...
		for (int i = 1; i <= searchTimes; i++) {
			...
			// 掉落物品
			List<ItemInfo> searchItems = NewTreasureRaiderDropCfg2.treasureRaiderDropItem(searchType, totalTimes + 1);
			...
			awards.rewardTakeAffectAndPush(player, Action.NEW_TREASURE_RAIDER2, 0);
		}

		HPNewTreasureRaiderInfoSync2.Builder infoBuilder = NewTreasureRaiderInfoHandler2.generateInfo(player);
		...
		player.sendProtocol(Protocol.valueOf(HP.code.NEW_TREASURE_RAIDER_SEARCH2_S_VALUE, builder));
		// 更新玩家的活动数据
		player.getPlayerData().updateActivity(activityId, timeCfg.getStageId());
		// BI 日志 (freeTimes 本次寻宝开始前今日剩余的免费次数)
		BehaviorLogger.log4Platform(player, Action.NEW_TREASURE_RAIDER2, Params.valueOf("searchType", searchType),
