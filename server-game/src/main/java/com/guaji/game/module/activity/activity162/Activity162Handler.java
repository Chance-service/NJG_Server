 ...
 * 成長禮物領取協定
 */
public class Activity162Handler implements IProtocolHandler {
	...
	public boolean onProtocol(AppObj appObj, Protocol protocol) {
		...
		// 检测活动是否开放
		int activityId = Const.ActivityId.ACTIVITY162_GROWTH_LV_VALUE;
		...
		Activity162Status status = ActivityUtil.getActivityStatus(player.getPlayerData(), activityId, stageId, Activity162Status.class);
	...
	private static void GetRward(Protocol protocol,int action,int cfgId,Player player, Activity162Status status) {
		
		GrowthLVCfg cfg = ConfigManager.getInstance().getConfigByKey(GrowthLVCfg.class, cfgId);
		...
		if (player.getLevel() < cfg.getLvNeed()) {
			player.sendError(protocol.getType(), Status.error.PARAMS_INVALID_VALUE);
			return;
		}
		...
		awards.rewardTakeAffectAndPush(player, Action.ACTIVITY162_GROWTH_LV, 2,TapDBSource.Pass_Port,
		...
		player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY162_Growth_LV_S, response));
		
//		BehaviorLogger.log4Platform(player, Action.ACTIVITY162_GROWTH_LV, Params.valueOf("cfgId", cfgId),
//				Params.valueOf("free", free),
//				Params.valueOf("cost", cost),
//				Params.valueOf("allaward", allaward));

		BehaviorLogger.log4Service(player, Source.USER_OPERATION, Action.ACTIVITY162_GROWTH_LV,Params.valueOf("cfgId", cfgId),
				Params.valueOf("free", free),
				Params.valueOf("cost", cost),
				Params.valueOf("allaward", allaward));
	}
	 ...
	 * @param status
	 */
	private static void SyncInfo(int action,Player player, Activity162Status status) {
		GrowthPassRes.Builder builder = getBuilder(action,status);
		player.sendProtocol(Protocol.valueOf(HP.code.ACTIVITY162_Growth_LV_S, builder));
	}
	
	private static GrowthPassRes.Builder getBuilder(int action,Activity162Status status) {
		// 返回包
