 ...
 * 新神将投放掉落组配表
 */
@ConfigManager.XmlResource(file = "xml/releaseURMultip.xml", struct = "map")
public class ReleaseURMultipCfg extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, ReleaseURMultipCfg> itemsMap = new TreeMap<Integer, ReleaseURMultipCfg>();

	public ReleaseURMultipCfg() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			ReleaseURMultipCfg cfg = ConfigManager.getInstance().getConfigByKey(ReleaseURMultipCfg.class, id);
			if (cfg == null) {
				return false;
			}
