 ...
 * 新神将投放奖池
 */
@ConfigManager.XmlResource(file = "xml/releaseURDrop173.xml", struct = "map")
public class ReleaseURDropCfg173 extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static ReleaseURDropCfg173 RandomReward(int PoolId) {
		
		ReleaseURDropCfg173 acfg = null;
		...
		if ((PoolId == POOL_TYPE_SEARCH)||(PoolId == POOL_TYPE_BOX)) {
		    ...
		    acfg = ConfigManager.getInstance().getConfigByKey(ReleaseURDropCfg173.class,rid);
		}
		return acfg;
	}
	...
	public static List<ItemInfo> getDropItem(int dropId) {
		...
		try {
			ReleaseURDropCfg173 dropCfg = ConfigManager.getInstance().getConfigByKey(ReleaseURDropCfg173.class, dropId);
