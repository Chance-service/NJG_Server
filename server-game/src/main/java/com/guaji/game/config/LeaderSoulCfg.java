...
@ConfigManager.XmlResource(file = "xml/LeaderSoul.xml", struct = "map")
public class LeaderSoulCfg extends ConfigBase {
	 ...
	 * @param level
	 */
	public static LeaderSoulCfg getCfg(int group, int level) {
		Map<Object, LeaderSoulCfg> map = ConfigManager.getInstance().getConfigMap(LeaderSoulCfg.class);
		for (Entry<Object, LeaderSoulCfg> entry : map.entrySet()) {
	...
	public static LeaderSoulCfg getSoulCfg(int id) {
		return ConfigManager.getInstance().getConfigByKey(LeaderSoulCfg.class, id);
	}
