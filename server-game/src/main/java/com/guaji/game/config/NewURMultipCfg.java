...
@ConfigManager.XmlResource(file = "xml/newURMultip.xml", struct = "map")
public class NewURMultipCfg extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, NewURMultipCfg> itemsMap = new TreeMap<Integer, NewURMultipCfg>();

	public NewURMultipCfg() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			NewURMultipCfg cfg = ConfigManager.getInstance()
					.getConfigByKey(NewURMultipCfg.class, id);
			if (cfg == null) {
				return false;
			}
