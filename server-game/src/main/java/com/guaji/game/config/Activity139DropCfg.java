...
@ConfigManager.XmlResource(file = "xml/Activity139DropCfg.xml", struct = "map")
public class Activity139DropCfg extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static final int POOL_TYPE_SKIN_SEARCH = 3;
	public static final int POOL_TYPE_SKIN_BOX = 4;
	...
	public static int GetPoolSize() {
		return poolItemsMap.keySet().size();
	}

	public static int getDropID(int seachType, int totalTimes) {
		int dropId = 0;
		try {
			...
			if (specail) {
				...
				seachType=(seachType-1)*2+1;
				// 从特殊掉落奖池中随机
				totalRate = poolTotalRateMap.get(seachType);
				dropId = calcDropId(seachType, GuaJiRand.randInt(1, totalRate));
			} else {
				// 从普通掉落奖池中随机
				seachType=(seachType-1)*2;
				TreeMap<Integer, Integer> poolSpecialTimes = limitTimesMap.get(seachType);
	...
	public static List<ItemInfo> treasureRaiderDropItem(int seachType, int totalTimes) {
		List<ItemInfo> items = new ArrayList<ItemInfo>();
		try {
			int dropId = getDropID(seachType, totalTimes);
			Activity139DropCfg dropCfg = ConfigManager.getInstance().getConfigByKey(Activity139DropCfg.class, dropId);
	 ...
	 * @return
	 */
	public static List<Integer> calcCanDropMercenaryId() {

		List<Integer> mercenaryIdList = new ArrayList<>();
		poolItemsMap.forEach((key, value) -> {
			if (key != POOL_TYPE_NONE) {
				Optional.ofNullable(value).ifPresent(t -> {
					t.keySet().stream().forEach(dorpId -> {
						Activity139DropCfg dropCfg = ConfigManager.getInstance()
								.getConfigByKey(Activity139DropCfg.class, dorpId);
						Optional.ofNullable(dropCfg).ifPresent(cfg -> {
							Optional.ofNullable(AwardItems.valueOf(cfg.getRewards())).ifPresent(awdItems -> {
								awdItems.getAwardItems().stream()
										.filter(awditem -> awditem.getType() / 10000 == Const.itemType.SOUL_VALUE)
										.findFirst().ifPresent(item -> {
											mercenaryIdList.add(item.getId());
										});
							});
						});
					});
		...
		return mercenaryIdList;
	}

	/**
	 * 宝箱掉落
	 * 
	 * @param curTotalBoxTimes 当前总开宝箱次数
	 * @return
	 */
	 ...
	 * @return
	 */
	public static int getLimitTimes(int seachType, int curTotalTimes) {
		
		int poolId=(seachType-1)*2+1;
	 ...
	 * @return
	 */
	public static int getLimitTimes(TreasureSearchType seachType, int curTotalTimes) {
		TreeMap<Integer, Integer> poolSpecialTimes = limitTimesMap.get(POOL_TYPE_BOX);
		Integer limitTime = poolSpecialTimes.higherKey(curTotalTimes);
		if (limitTime == null) {
			return -1;
	...
	public static int getWillActRoleId() {
		// 从特殊掉落奖池中随机
		try {
			int totalRate = poolTotalRateMap.get(POOL_TYPE_BOX);
			int dropId = calcDropId(POOL_TYPE_BOX, GuaJiRand.randInt(1, totalRate));
			Activity139DropCfg dropCfg = ConfigManager.getInstance().getConfigByKey(Activity139DropCfg.class, dropId);
			String[] items = dropCfg.getRewards().split("_");
			if (items.length != 3)
				return 0;
			else
