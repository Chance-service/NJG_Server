...
@ConfigManager.XmlResource(file = "xml/obonDrop.xml", struct = "map")
public class ObonDropCfg extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes){
		...
		for(int i = 1; i <= searchTimes; i++){
			try {
				...
				ObonDropCfg dropCfg = ConfigManager.getInstance().getConfigByKey(ObonDropCfg.class, dropId);
	...
	public static List<ItemInfo> obonDropItem(int totalTimes)
	{
		List<ItemInfo> items = new ArrayList<ItemInfo>();
		try {
