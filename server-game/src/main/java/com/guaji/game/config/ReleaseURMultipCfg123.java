 ...
 * 新神将投放掉落组配表
 */
@ConfigManager.XmlResource(file = "xml/releaseURMultip123.xml", struct = "map")
public class ReleaseURMultipCfg123 extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, ReleaseURMultipCfg123> itemsMap = new TreeMap<Integer, ReleaseURMultipCfg123>();

	public ReleaseURMultipCfg123() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			ReleaseURMultipCfg123 cfg = ConfigManager.getInstance().getConfigByKey(ReleaseURMultipCfg123.class, id);
			if (cfg == null) {
				return false;
			}
