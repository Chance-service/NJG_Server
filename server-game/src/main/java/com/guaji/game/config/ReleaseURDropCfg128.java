 ...
 * 新神将投放奖池
 */
@ConfigManager.XmlResource(file = "xml/releaseURDrop128.xml", struct = "map")
public class ReleaseURDropCfg128 extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes) {
		...
		for (int i = 1; i <= searchTimes; i++) {
			try {
				...
				ReleaseURDropCfg128 dropCfg = ConfigManager.getInstance().getConfigByKey(ReleaseURDropCfg128.class,
						dropId);
	...
	public static List<ItemInfo> treasureRaiderDropItem(int totalTimes) {
		...
		try {
			...
			ReleaseURDropCfg128 dropCfg = ConfigManager.getInstance().getConfigByKey(ReleaseURDropCfg128.class, dropId);
			AwardItems awards = AwardItems.valueOf(dropCfg.getRewards());
			items.addAll(awards.getAwardItemInfos());
		} catch (Exception e) {
			MyException.catchException(e);
