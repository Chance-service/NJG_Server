...
public class ActivityMultipCfg extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, ActivityMultipCfg> itemsMap = new TreeMap<Integer, ActivityMultipCfg>();

	public ActivityMultipCfg() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			ActivityMultipCfg cfg = ConfigManager.getInstance()
					.getConfigByKey(ActivityMultipCfg.class, id);
			if (cfg == null) {
				return false;
			}
