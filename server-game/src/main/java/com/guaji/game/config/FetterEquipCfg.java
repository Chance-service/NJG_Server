 ...
 *
 */
@ConfigManager.XmlResource(file = "xml/fetterEquip.xml", struct = "map")
public class FetterEquipCfg extends ConfigBase {
	/**
	 * 配置id
	 */
	@Id
	...
	public String getNewAttribute(MutualEntity mutualEntity) {
		int star = getGroupMinStar(mutualEntity);
	 ...
	 * @return
	 */
	public int getGroupMinStar(MutualEntity mutualEntity) {
		int star = 13;
		for(int i = 0; i < archiveIds.size(); i++){
			int itemId = archiveIds.get(i);
			if (!mutualEntity.getStarMap().containsKey(itemId)){
				break;
			}

			star = Math.min(mutualEntity.getStarMap().get(itemId),star);
		}
		return star;
	}
