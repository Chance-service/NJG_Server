...
@ConfigManager.XmlResource(file = "xml/ElementSoul.xml", struct = "map")
public class ElementSoulCfg extends ConfigBase {
	 ...
	 * @param level
	 */
	public static ElementSoulCfg getCfg(int group, int level) {
		Map<Object, ElementSoulCfg> map = ConfigManager.getInstance().getConfigMap(ElementSoulCfg.class);
		for (Entry<Object, ElementSoulCfg> entry : map.entrySet()) {
	...
	public static ElementSoulCfg getElementSoulCfg(int id) {
		return ConfigManager.getInstance().getConfigByKey(ElementSoulCfg.class, id);
	}
