 ...
 * 新神将投放奖池
 */
@ConfigManager.XmlResource(file = "xml/releaseURDrop.xml", struct = "map")
public class ReleaseURDropCfg extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes) {
		...
		for (int i = 1; i <= searchTimes; i++) {
			try {
				...
				ReleaseURDropCfg dropCfg = ConfigManager.getInstance().getConfigByKey(ReleaseURDropCfg.class, dropId);
