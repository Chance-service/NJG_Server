...
@ConfigManager.XmlResource(file = "xml/newTreasureRaiderDrop2.xml", struct = "map")
public class NewTreasureRaiderDropCfg2 extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public NewTreasureRaiderDropCfg2() {
		id = 0;
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes) {
		...
		for (int i = 1; i <= searchTimes; i++) {
			try {
				...
				NewTreasureRaiderDropCfg2 dropCfg = ConfigManager.getInstance()
						.getConfigByKey(NewTreasureRaiderDropCfg2.class, dropId);
	...
	public static int getDropID(TreasureSearchType seachType, int totalTimes) {
		int dropId = 0;
		try {
			if (seachType == TreasureSearchType.SEARCHTYPE_BASIC) {
				int totalRate = poolTotalRateMap.get(POOL_TYPE_SEARCH);
				...
				int totalRate = poolTotalRateMap.get(POOL_TYPE_SKIN_SEARCH);
				boolean specail = totalTimes % 10 == 0 ? true : false;
				if (specail) {
					// 从特殊掉落奖池中随机
					totalRate = poolTotalRateMap.get(POOL_TYPE_SKIN_BOX);
					dropId = calcDropId(POOL_TYPE_SKIN_BOX, GuaJiRand.randInt(1, totalRate));
				} else {
					// 从普通掉落奖池中随机
					TreeMap<Integer, Integer> poolSpecialTimes = limitTimesMap.get(POOL_TYPE_SKIN_SEARCH);
					if (poolSpecialTimes.containsKey(totalTimes)) {
						dropId = poolSpecialTimes.get(totalTimes);
					} else {
						totalRate = poolTotalRateMap.get(POOL_TYPE_SKIN_SEARCH);
						dropId = calcDropId(POOL_TYPE_SKIN_SEARCH, GuaJiRand.randInt(1, totalRate));
					}
				}
			}
	...
	public static List<ItemInfo> treasureRaiderDropItem(TreasureSearchType seachType, int totalTimes) {
		List<ItemInfo> items = new ArrayList<ItemInfo>();
		try {
	...
	public static int getLimitTimes(TreasureSearchType seachType, int curTotalTimes) {
		if (seachType == TreasureSearchType.SEARCHTYPE_BASIC) {
			...
			TreeMap<Integer, Integer> poolSpecialTimes = limitTimesMap.get(POOL_TYPE_SKIN_BOX);
			Integer limitTime = poolSpecialTimes.higherKey(curTotalTimes);
			if (limitTime == null) {
				return -1;
			} else {
