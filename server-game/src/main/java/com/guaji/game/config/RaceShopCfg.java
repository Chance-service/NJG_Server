...
@ConfigManager.XmlResource(file = "xml/raceShop.xml", struct = "list")
public class RaceShopCfg extends ConfigBase {
	 ...
	 * 各群組cfg
	 */
	private static Map<Integer,List<RaceShopCfg>> groupbyCfg;
	...
	protected boolean assemble() {
		...
		if (groupbyCfg.containsKey(this.getGroup()))
		{
			...
			List<RaceShopCfg> clist = new ArrayList<>();
			clist.add(this);
	 ...
	 * @return
	 */
	public static DailyShopItem getShopInfo(RaceShopCfg cfg, int id) {
	 ...
	 * 取得所有商品
	 */
	public static List<RaceShopCfg> getShopCfgs(){
		List<RaceShopCfg> aList = ConfigManager.getInstance().getConfigList(RaceShopCfg.class);
		return aList ;
	}
	 ...
	 * @return
	 */
	public static RaceShopCfg getCfgbyIndex(int idx) {
		 List<RaceShopCfg> alist = getShopCfgs();
		 for (RaceShopCfg acfg : alist) {
			 if (idx == acfg.getId()) {
				 return acfg;
			 }
		 }
	...
	public static Map<Integer,List<RaceShopCfg>> getGroupbyCfg(){
		return groupbyCfg;
	}
}
