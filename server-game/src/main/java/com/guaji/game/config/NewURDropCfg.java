...
@ConfigManager.XmlResource(file = "xml/newURDrop.xml", struct = "map")
public class NewURDropCfg extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes){
		...
		for(int i = 1; i <= searchTimes; i++){
			try {
				...
				NewURDropCfg dropCfg = ConfigManager.getInstance().getConfigByKey(NewURDropCfg.class, dropId);
	...
	public static List<ItemInfo> newURDropItem(int totalTimes)
	{
		List<ItemInfo> items = new ArrayList<ItemInfo>();
		try {
