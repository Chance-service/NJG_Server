 ...
 * 新神将投放奖池
 */
@ConfigManager.XmlResource(file = "xml/releaseURDrop123.xml", struct = "map")
public class ReleaseURDropCfg123 extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes) {
		...
		for (int i = 1; i <= searchTimes; i++) {
			try {
				...
				ReleaseURDropCfg123 dropCfg = ConfigManager.getInstance().getConfigByKey(ReleaseURDropCfg123.class, dropId);
