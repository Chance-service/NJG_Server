...
@ConfigManager.XmlResource(file = "xml/ClassSoul.xml", struct = "map")
public class ClassSoulCfg extends ConfigBase {
	 ...
	 * @param level
	 */
	public static ClassSoulCfg getCfg(int group, int level) {
		Map<Object, ClassSoulCfg> map = ConfigManager.getInstance().getConfigMap(ClassSoulCfg.class);
		for (Entry<Object, ClassSoulCfg> entry : map.entrySet()) {
	...
	public static ClassSoulCfg getClassSoulCfg(int id) {
		return ConfigManager.getInstance().getConfigByKey(ClassSoulCfg.class, id);
	}
