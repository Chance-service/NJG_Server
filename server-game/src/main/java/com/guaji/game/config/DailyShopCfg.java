...
@ConfigManager.XmlResource(file = "xml/dailyShop.xml", struct = "list")
public class DailyShopCfg extends ConfigBase {
	 ...
	 * 各群組cfg
	 */
	private static Map<Integer,List<DailyShopCfg>> groupbyCfg;
	/**
	...
	protected boolean assemble() {
		...
		if (groupbyCfg.containsKey(this.getGroup()))
		{
			...
			List<DailyShopCfg> clist = new ArrayList<>();
			clist.add(this);
	 ...
	 * @return
	 */
	public static DailyShopItem getDailyShopInfo(DailyShopCfg cfg, int id) {
	 ...
	 * 取得所有商品
	 */
	public static List<DailyShopCfg> getDailyShopCfgs(){
		List<DailyShopCfg> aList = ConfigManager.getInstance().getConfigList(DailyShopCfg.class);
		return aList ;
	}
	 ...
	 * 取得單一商品byId
	 */
	public static DailyShopCfg getShopCfgById(int id){
		return DailyShopCfg.getDailyShopCfgs().stream().filter(cfg -> cfg.getId() == id).findFirst().orElse(null);
	...
	public static Map<Integer,List<DailyShopCfg>> getGroupbyCfg(){
		return groupbyCfg;
	}
