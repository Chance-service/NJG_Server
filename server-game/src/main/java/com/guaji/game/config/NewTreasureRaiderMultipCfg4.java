...
@ConfigManager.XmlResource(file = "xml/newTreasureRaiderMultip4.xml", struct = "map")
public class NewTreasureRaiderMultipCfg4 extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, NewTreasureRaiderMultipCfg4> itemsMap = new TreeMap<Integer, NewTreasureRaiderMultipCfg4>();

	public NewTreasureRaiderMultipCfg4() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			NewTreasureRaiderMultipCfg4 cfg = ConfigManager.getInstance()
					.getConfigByKey(NewTreasureRaiderMultipCfg4.class, id);
			if (cfg == null) {
				return false;
			}
