...
@ConfigManager.XmlResource(file = "xml/newTreasureRaiderDrop120.xml", struct = "map")
public class NewTreasureRaiderDropCfg3 extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public NewTreasureRaiderDropCfg3(){
		id = 0;
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes){
		...
		for(int i = 1; i <= searchTimes; i++){
			try {
				...
				NewTreasureRaiderDropCfg3 dropCfg = ConfigManager.getInstance().getConfigByKey(NewTreasureRaiderDropCfg3.class, dropId);
