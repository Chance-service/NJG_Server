 ...
 * 新神将投放奖池
 */
@ConfigManager.XmlResource(file = "xml/releaseURDrop146.xml", struct = "map")
public class ReleaseURDropCfg146 extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static final int POOL_TYPE_NEWBIE = 3;
	/**
	 * 掉落物组ID
	 */
	@Id
	...
	protected boolean assemble() {
		...
		for (int pool = POOL_TYPE_SEARCH ; pool <= POOL_TYPE_NEWBIE ; pool++) {
	...
	public static ReleaseURDropCfg146 RandomReward(int PoolId) {
		
		ReleaseURDropCfg146 acfg = null;
		
		if ((PoolId >= POOL_TYPE_SEARCH)&&(PoolId <= POOL_TYPE_NEWBIE)) {
		    int rid = GuaJiRand.randonWeightObject(idMap.get(PoolId),drawRateMap.get(PoolId));	
		
		    acfg = ConfigManager.getInstance().getConfigByKey(ReleaseURDropCfg146.class,rid);
		}
		return acfg;
	}
	...
	public static List<ItemInfo> getDropItem(int dropId) {
		...
		try {
			ReleaseURDropCfg146 dropCfg = ConfigManager.getInstance().getConfigByKey(ReleaseURDropCfg146.class, dropId);
