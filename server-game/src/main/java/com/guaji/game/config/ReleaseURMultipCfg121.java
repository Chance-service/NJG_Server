 ...
 * 新神将投放掉落组配表
 */
@ConfigManager.XmlResource(file = "xml/releaseURMultip121.xml", struct = "map")
public class ReleaseURMultipCfg121 extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, ReleaseURMultipCfg121> itemsMap = new TreeMap<Integer, ReleaseURMultipCfg121>();

	public ReleaseURMultipCfg121() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			ReleaseURMultipCfg121 cfg = ConfigManager.getInstance().getConfigByKey(ReleaseURMultipCfg121.class, id);
			if (cfg == null) {
				return false;
			}
