...
@ConfigManager.XmlResource(file = "xml/shootRewardA.xml", struct = "map")
public class ShootRewardCfgA extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static ShootRewardCfgA getShootRewardA(int poolType, int shootTotalCount) {
		...
		ShootRewardCfgA dropCfg = ConfigManager.getInstance().getConfigByKey(ShootRewardCfgA.class, dropId);
		return dropCfg;
	}
	 ...
	 * 获取射击物品B
	 */
	public static ShootRewardCfgB getShootRewardB(int poolType, int shootTotalCount) {
		int totalRate = poolTotalRateMap.get(poolType);
		TreeMap<Integer, Integer> poolSpecialTimes = limitTimesMap.get(poolType);
		int dropId = 0;
		int times = shootTotalCount + 1;
		...
		ShootRewardCfgB dropCfg = ConfigManager.getInstance().getConfigByKey(ShootRewardCfgB.class, dropId);
		return dropCfg;
	}
	 ...
	 * @return
	 */
	public static List<ItemInfo> getShootRewards(int poolType, int curTypeTotalTimes, int shootCount) {
		int totalRate = poolTotalRateMap.get(poolType);
		TreeMap<Integer, Integer> poolSpecialTimes = limitTimesMap.get(poolType);
		...
		for (int i = 1; i <= shootCount; i++) {
			try {
				...
				TreasureRaiderDropCfg dropCfg = ConfigManager.getInstance().getConfigByKey(TreasureRaiderDropCfg.class,
						dropId);
