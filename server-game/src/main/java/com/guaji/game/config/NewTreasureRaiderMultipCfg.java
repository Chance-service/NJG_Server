...
@ConfigManager.XmlResource(file = "xml/newTreasureRaiderMultip.xml", struct = "map")
public class NewTreasureRaiderMultipCfg extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, NewTreasureRaiderMultipCfg> itemsMap = new TreeMap<Integer, NewTreasureRaiderMultipCfg>();

	public NewTreasureRaiderMultipCfg() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			NewTreasureRaiderMultipCfg cfg = ConfigManager.getInstance()
					.getConfigByKey(NewTreasureRaiderMultipCfg.class, id);
			if (cfg == null) {
				return false;
			}
