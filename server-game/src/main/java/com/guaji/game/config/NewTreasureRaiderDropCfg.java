...
@ConfigManager.XmlResource(file = "xml/newTreasureRaiderDrop.xml", struct = "map")
public class NewTreasureRaiderDropCfg extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public NewTreasureRaiderDropCfg(){
		id = 0;
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes){
		...
		for(int i = 1; i <= searchTimes; i++){
			try {
				...
				NewTreasureRaiderDropCfg dropCfg = ConfigManager.getInstance().getConfigByKey(NewTreasureRaiderDropCfg.class, dropId);
