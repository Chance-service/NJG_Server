...
@ConfigManager.XmlResource(file = "xml/newTreasureRaiderMultip2.xml", struct = "map")
public class NewTreasureRaiderMultipCfg2 extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, NewTreasureRaiderMultipCfg2> itemsMap = new TreeMap<Integer, NewTreasureRaiderMultipCfg2>();

	public NewTreasureRaiderMultipCfg2() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			NewTreasureRaiderMultipCfg2 cfg = ConfigManager.getInstance()
					.getConfigByKey(NewTreasureRaiderMultipCfg2.class, id);
			if (cfg == null) {
				return false;
			}
