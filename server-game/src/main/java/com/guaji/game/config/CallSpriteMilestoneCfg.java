...
@ConfigManager.XmlResource(file = "xml/releaseURMilestone154.xml", struct = "list")
public class CallSpriteMilestoneCfg extends ConfigBase {
	/**
	 * 配置id;
	 */
	@Id
	...
	public static CallSpriteMilestoneCfg getMileStoneCfg() {
		CallSpriteMilestoneCfg Cfg = ConfigManager.getInstance().getConfigByIndex(CallSpriteMilestoneCfg.class,0);
		return Cfg;
	}
	...
	protected boolean assemble() {
		...
		if (PointList.size() != rewardList.size()) {
			Log.errPrintln("releaseURMilestone154.xml reward size error");
			return false;
		}
