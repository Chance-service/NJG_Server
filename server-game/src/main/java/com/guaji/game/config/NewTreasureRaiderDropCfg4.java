...
@ConfigManager.XmlResource(file = "xml/newTreasureRaiderDrop4.xml", struct = "map")
public class NewTreasureRaiderDropCfg4 extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public NewTreasureRaiderDropCfg4() {
		id = 0;
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes) {
		...
		for (int i = 1; i <= searchTimes; i++) {
			try {
				...
				NewTreasureRaiderDropCfg4 dropCfg = ConfigManager.getInstance()
						.getConfigByKey(NewTreasureRaiderDropCfg4.class, dropId);
