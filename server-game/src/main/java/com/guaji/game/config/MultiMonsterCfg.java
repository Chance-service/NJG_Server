...
@ConfigManager.XmlResource(file = "xml/multiMonster.xml", struct = "map")
public class MultiMonsterCfg extends ConfigBase {
	/**
	 * 配置id
	 */
	@Id
	...
	public int getFight() {
		float fightValue = (attribute.getValue(Const.attr.MINDMG) + attribute.getValue(Const.attr.MAXDMG)) * 1.3f
				+ attribute.getValue(Const.attr.DODGE) * 5.0f + attribute.getValue(Const.attr.HP) * 0.1f + attribute.getValue(Const.attr.MP) * 0.1f
				+ attribute.getValue(Const.attr.RESILIENCE) * 1.0f + attribute.getValue(Const.attr.CRITICAL) * 1.0f
				+ attribute.getValue(Const.attr.PHYDEF) * 1.0f + attribute.getValue(Const.attr.MAGDEF) * 1.0f
				+ attribute.getValue(Const.attr.ARMOR) * 1.0f + attribute.getValue(Const.attr.HIT) * 1.0f;
