...
@ConfigManager.XmlResource(file = "xml/newTreasureRaiderMultip120.xml", struct = "map")
public class NewTreasureRaiderMultipCfg3 extends ConfigBase {
	/**
	 * 掉落物组ID
	 */
	@Id
	 ...
	 * 奖池掉落物权重分布 Map
	 */
	private static TreeMap<Integer, NewTreasureRaiderMultipCfg3> itemsMap = new TreeMap<Integer, NewTreasureRaiderMultipCfg3>();

	public NewTreasureRaiderMultipCfg3() {
		id = 0;
		startlucky = 0;
		endlucky = 0;
		rate = 0;
	...
	public static boolean isActiveMultiple(int luckyValue) {
		...
		try {
			NewTreasureRaiderMultipCfg3 cfg = ConfigManager.getInstance()
					.getConfigByKey(NewTreasureRaiderMultipCfg3.class, id);
			if (cfg == null) {
				return false;
			}
