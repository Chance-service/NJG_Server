...
@ConfigManager.XmlResource(file = "xml/starSoul.xml", struct = "map")
public class StarSoulCfg extends ConfigBase {
	 ...
	 * @param level
	 */
	public static StarSoulCfg getCfg(int group, int level) {
		Map<Object, StarSoulCfg> map = ConfigManager.getInstance().getConfigMap(StarSoulCfg.class);
		for (Entry<Object, StarSoulCfg> entry : map.entrySet()) {
	...
	public static StarSoulCfg getStarSoulCfg(int id) {
		return ConfigManager.getInstance().getConfigByKey(StarSoulCfg.class, id);
	}
