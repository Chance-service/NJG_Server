 ...
 * @version 创建时间：2019年5月28日 上午10:55:33 类说明
 */
@ConfigManager.XmlResource(file = "xml/activityURDrop_127.xml", struct = "map")
public class ActivityURDrop127 extends ConfigBase {
	/**
	 * 奖池ID定义
	 */
	...
	public static List<ItemInfo> treasureRaiderDropItems(int curTotalSearchTimes, int searchTimes) {
		...
		for (int i = 1; i <= searchTimes; i++) {
			try {
				...
				ActivityURDrop127 dropCfg = ConfigManager.getInstance().getConfigByKey(ActivityURDrop127.class,
						dropId);
	...
	public static List<ItemInfo> treasureRaiderDropItem(int totalTimes) {
		...
		try {
			...
			ActivityURDrop127 dropCfg = ConfigManager.getInstance().getConfigByKey(ActivityURDrop127.class, dropId);
			AwardItems awards = AwardItems.valueOf(dropCfg.getRewards());
			items.addAll(awards.getAwardItemInfos());
		} catch (Exception e) {
			MyException.catchException(e);
