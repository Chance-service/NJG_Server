...
@SingleRank(type = RankType.WING_GREEN_TIME_RANK, maxRankNum = GsConst.MAX_WING_RANK_NUM)
public class WingGreenRank extends WingRank {
	private String sql = "from PlayerWingsEntity where greenTime > 0 and invalid = 0 order by greenTime asc";
	...
	public void setRankParam(PlayerWingsEntity playerWingsEntity, PlayerWingRank wingRank) {
		...
		wingRank.setUseTime(playerWingsEntity.getGreenTime());
	}
}
