...
@Entity
@Table(name = "guild_shop")
public class GuildShopEntity extends DBEntity {
	@Id
	...
	@Transient
	private Map<Integer, DailyShopItem> GuildShopMap;
	...
	public GuildShopEntity() {
		this.createTime = GuaJiTime.getCalendar().getTime();
		this.GuildShopMap = new HashMap<Integer, DailyShopItem>(8);
	}

	public void convert() {
		if (getShopItemMapStr() != null) {
			this.GuildShopMap = GsonUtil.getJsonInstance().fromJson(this.getShopItemMapStr(), new TypeToken<HashMap<Integer, DailyShopItem>>() {
			}.getType());
		}
	}
	...
	public static GuildShopEntity valueOf(Player player) {
		GuildShopEntity guildShopEntity = new GuildShopEntity();
		guildShopEntity.playerId = player.getId();
		GuildMarket.getInstance().shopItemInfo(player, guildShopEntity,true);
	...
	public DailyShopItem getDailyShopItem(int id) {
		return this.GuildShopMap.get(id);
	}
