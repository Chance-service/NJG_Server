...
public class SkillHandler_3001 extends SkillHandler{

	public SkillHandler_3001(int skillId) {
		super(skillId);
	...
	public boolean checkSkillAction(Battlefield battlefield, NewBattleRole ActRole ,NewBattleRole targetRole
		...
		//觸動被動 Passive_ID_10003 補隊友然後會補自己
		
		if (targetRole.IsEnemy(ActRole.getPos())) {
			return false;
		}
		
		if ((ActRole.Istrigger(GsConst.TriggerType.SKILL,SkillType.Passive_ID_10003))&&(targetRole.IsMyself(ActRole.getPos()))) { // 不在這個地方檢查
			return true; 
		}
