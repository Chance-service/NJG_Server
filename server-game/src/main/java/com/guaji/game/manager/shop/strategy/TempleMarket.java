...
import com.guaji.game.config.TempleShopCfg;
 ...
 *
 */
public class TempleMarket extends ShopScervice<TempleShopCfg> {

	private static final TempleMarket instance = new TempleMarket();

	public static TempleMarket getInstance() {
		return instance;
	}
	 ...
	 * @param luckyScore
	 */
	public List<TempleShopCfg> shopItemInfo(Player player, TempleShopEntity entity,boolean isCreate) {

		entity.getShopMap().clear();
		// vip等级对应物品数量
		//int showItemCount = this.getMaxShopItemCount(player.getVipLevel());
		// 等级对应物品集
		List<TempleShopCfg> ShopList = TempleShopCfg.getShopCfgs();
		// 商品集合clone
		List<TempleShopCfg> ShopCfgsClone = new ArrayList<TempleShopCfg>(ShopList);

		// 商城显示集合
		//List<TempleShopCfg> crystalItemList = this.getItemList(DailyShopCfgsClone, showItemCount);

		for (int i = 0; i < ShopCfgsClone.size(); i++) {
			DailyShopItem ShopItem = TempleShopCfg.getShopInfo(ShopCfgsClone.get(i), i + 1);
			entity.addShopItem(ShopItem);
		}
	...
	public void sendShopBuilder(Player player) {
		...
		TempleShopEntity entity = player.getPlayerData().loadTempleShopEntity();
		...
		for (DailyShopItem item : ItemMap.values()) {
			...
			TempleShopCfg acfg = TempleShopCfg.getCfgbyIndex(item.getCfgIndex());
			builder.addData(this.createDisplayData(DataType.ITEM_LEFT,item.getLeft()));
			builder.addData(this.createDisplayData(DataType.LEVEL_LIMIT,acfg.getLv()));
		}
...
//	}
	...
	public void shopBuyLogic(Player player, BuyShopItemsRequest request) {
		...
		TempleShopEntity ShopEntity = player.getPlayerData().loadTempleShopEntity();
		...
		TempleShopCfg shopcfg = TempleShopCfg.getCfgbyIndex(shopItem.getCfgIndex());
		...
		if (player.getLevel() < shopcfg.getLv()) {
	...
	public ShopType getShopType() {
		return Const.ShopType.TEMPLE_MARKET;
	}
