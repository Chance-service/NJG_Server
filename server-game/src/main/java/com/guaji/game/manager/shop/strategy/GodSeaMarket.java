...
import com.guaji.game.config.GodSeaShopCfg;
 ...
 *
 */
public class GodSeaMarket extends ShopScervice<GodSeaShopCfg> {

	private static final GodSeaMarket instance = new GodSeaMarket();

	public static GodSeaMarket getInstance() {
		return instance;
	}
	 ...
	 * @param luckyScore
	 */
	public List<GodSeaShopCfg> shopItemInfo(Player player, GodSeaShopEntity entity,boolean isCreate) {

		entity.getShopMap().clear();
		// vip等级对应物品数量
		//int showItemCount = this.getMaxShopItemCount(player.getVipLevel());
		// 等级对应物品集
		List<GodSeaShopCfg> ShopList = GodSeaShopCfg.getShopCfgs();
		// 商品集合clone
		List<GodSeaShopCfg> ShopCfgsClone = new ArrayList<GodSeaShopCfg>(ShopList);

		// 商城显示集合
		//List<GodSeaShopCfg> crystalItemList = this.getItemList(DailyShopCfgsClone, showItemCount);

		for (int i = 0; i < ShopCfgsClone.size(); i++) {
			DailyShopItem dailyShopItem = GodSeaShopCfg.getDailyShopInfo(ShopCfgsClone.get(i), i + 1);
	...
	public void sendShopBuilder(Player player) {
		...
		GodSeaShopEntity entity = player.getPlayerData().loadGodSeaShopEntity();
...
//	}
	...
	public void shopBuyLogic(Player player, BuyShopItemsRequest request) {
		...
		GodSeaShopEntity ShopEntity = player.getPlayerData().loadGodSeaShopEntity();
	...
	public ShopType getShopType() {
		return Const.ShopType.GODSEA_MARKET;
	}
