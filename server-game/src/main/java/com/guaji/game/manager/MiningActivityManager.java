...
import com.guaji.game.config.GoldMineRankCfg;
...
import com.guaji.game.entity.MiningRankActivityEntity;
import com.guaji.game.item.AwardItems;
import com.guaji.game.module.activity.activity165.Activity165Status;
import com.guaji.game.module.activity.activity165.Activity165Rank;
 ...
 * 挖礦積分排行管理
 */
public class MiningActivityManager extends AppObj {

	private final int maxRankNum = SysBasicCfg.getInstance().getActivity165RankMaxNum();
	private final int maxRewardNum = 50;
	 ...
	 * 活动ID
	 */
	private final int activityId = Const.ActivityId.ACTIVITY165_MINING_VALUE;
	 ...
	 * 当前活动对应的奖励发放数据
	 */
	private MiningRankActivityEntity  miningRankActivityEntity = null;
	 ...
	 * 所有玩家抽卡积分
	 */
	private List<Activity165Rank> rankList = new LinkedList<Activity165Rank>();
	 ...
	 * 所有玩家历史抽卡积分
	 */
	//private List<Activity165Rank> rankHistoryList = new LinkedList<Activity165Rank>();

	/**
	 * 历史数据及排行
	 */
	//private Map<Integer, Activity165Rank> allPlayerIdRankHistoryMap = new ConcurrentHashMap<Integer, Activity165Rank>();

	/**
	 * 全局对象, 便于访问
	 */
	private static MiningActivityManager instance = null;

	/**
	 * 获取全局实例对象
	 */
	public static MiningActivityManager getInstance() {
		return instance;
	}

	public MiningActivityManager(GuaJiXID xid) {
	...
	public void init() {
		...
		if (activityItem != null) {
			...
			if (timeConfig != null) {
				// 排行数据加载
				List<ActivityEntity<Activity165Status>> rankEntity = DBManager.getInstance().query(
						"from ActivityEntity where activityId = ? and stageId = ?", activityId,
						timeConfig.getStageId());
				List<Activity165Rank> allRankList = new LinkedList<Activity165Rank>();
				if (null != rankEntity && rankEntity.size() > 0) {
					Activity165Rank item = null;
					for (ActivityEntity<Activity165Status> activityEntity : rankEntity) {
						Activity165Status status = activityEntity.getActivityStatus(Activity165Status.class);
						if (status != null && status.getScore() > 0) {

							item = new Activity165Rank(activityEntity.getPlayerId(), status.getScore());
				...
				// 将排行数据存放到全局变量
				int selNum = SysBasicCfg.getInstance().getActivity165RankMaxNum() > allRankList.size()
						? allRankList.size()
						: SysBasicCfg.getInstance().getActivity165RankMaxNum();
				if (selNum > 0)
					this.rankList.addAll(allRankList.subList(0, selNum));

				int index = 0;
				for (Activity165Rank item : this.rankList) {
					allPlayerIdRankMap.put(item.getPlayerId(), index);
					index++;
				}
				// 排行数据加载
				List<MiningRankActivityEntity> RankEntityList = DBManager.getInstance()
						.query("from MiningRankActivityEntity where invalid = 0");

				if (RankEntityList != null && RankEntityList.size() != 0) {
					miningRankActivityEntity = RankEntityList.get(0);
				}
				// 排行数据加载
...
//				this.refreshHistoryData();
	...
	public void refresh() {
		...
		if (timeCfg == null || timeCfg.isEnd()) {
			this.miningRankActivityEntity = null;
		} else {
			if (this.miningRankActivityEntity == null) {
				this.miningRankActivityEntity = new MiningRankActivityEntity();
				long refreshTime =  GuaJiTime.getNextWeekAM0Date(); // 下周一零點
//				if (GuaJiTime.getMillisecond() >= refreshTime) {
//					refreshTime += 24 * 60 * 60 * 1000;
//				}
				this.miningRankActivityEntity.setRefreshTime(refreshTime);
				DBManager.getInstance().create(miningRankActivityEntity);
			}
		}
	 ...
	 * @return
	 */
	public List<Activity165RankItem.Builder> getRankTop(int topNumber) {

		Map<Integer, Integer> everyRankRepeatMap = new HashMap<Integer, Integer>();
		List<Integer> scoreList = new LinkedList<Integer>();
		int totalNum = 0;
		int rankNum = 1;
		List<Activity165RankItem.Builder> list = new LinkedList<>();
		...
		for (Integer key : everyRankRepeatMap.keySet()) {
			Activity165RankItem.Builder builder = Activity165RankItem.newBuilder();
...
//					new TypeToken<List<Activity165Rank>>() {
//					}.getType());
//			for (Activity165Rank item : this.rankHistoryList) {
//				allPlayerIdRankHistoryMap.put(item.getPlayerId(), item);
//			}
//		}
//	}

//	public Activity165Rank getPlayerHistoryRank(int playerId) {
//
//		Map<Integer, Activity165Rank> everyRankNumMap = new HashMap<Integer, Activity165Rank>();
//		List<Integer> scoreList = new LinkedList<Integer>();
//		int rankNum = 0;
//		synchronized (this.rankHistoryList) {
//			Activity165Rank temp = null;
...
//				}
//				temp = new Activity165Rank(this.rankHistoryList.get(i).getPlayerId(),
...
//	public List<Activity165RankItem.Builder> getRankHistoryTop() {
//
//		List<Activity165RankItem.Builder> list = new LinkedList<>();
...
//	}
	...
	public MiningRankActivityEntity getMiningRankActivityEntity() {
		return miningRankActivityEntity;
	}
...
//	}
	...
	public void updateRankSet(PlayerData playerData) {
		...
		synchronized (this.rankList) {
			...
			Activity165Status status = ActivityUtil.getActivityStatus(playerData, activityId, timeConfig.getStageId(),
					Activity165Status.class);
			...
			Activity165Rank playerStatusRank = new Activity165Rank(playerData.getId(), playerScore);
			int curIndex = 0;
			for (; curIndex < this.rankList.size(); curIndex++) {
				Activity165Rank rankItem = this.rankList.get(curIndex);
	...
	public boolean onTick() {
		...
		if (++tickIndex % 100 == 0) {
			...
			if (this.miningRankActivityEntity == null)
				return true;
			long nextFreshTime = this.miningRankActivityEntity.getRefreshTime();
			// 重置数据
			if (GuaJiTime.getMillisecond() > nextFreshTime) {
					logger.info("reset MiningActivityManager rankList before");
				synchronized (this.rankList) {
					logger.info("reset MiningActivityManager rankList after");
					long refreshTime = GuaJiTime.getNextWeekAM0Date(); // 下周一零點
...
//					}
					...
					// 更新历史排名
					this.miningRankActivityEntity.setRefreshTime(refreshTime);
					this.miningRankActivityEntity.notifyUpdate();
	...
	public boolean onMessage(Msg msg) {
		if (msg.getMsg() == GsConst.MsgType.MINING_RANK_ADD_SCORE) {
	...
	private void sendRankAwards(int stageId) {
		...
		for (Integer key : everyRankNumMap.keySet()) {
			GoldMineRankCfg cfg = ConfigManager.getInstance().getConfigByKey(GoldMineRankCfg.class,
			...
			MailManager.createMail(key, Mail.MailType.Reward_VALUE, GsConst.MailId.MINING_RANKMANAGER_MAIL, "挖礦積分排名奖励",
					everydayAward, date, String.valueOf(everyRankNumMap.get(key)));

		}

		// 生成历史排行
//		List<Activity165Rank> historyRankList = new LinkedList<Activity165Rank>();
//		Activity165Rank item = null;
//		for (int i = 0; i < this.rankList.size(); i++) {
//
//			item = new Activity165Rank(this.rankList.get(i).getPlayerId(), this.rankList.get(i).getScore(), i + 1);
