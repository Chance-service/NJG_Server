...
import com.guaji.game.config.SkinShopCfg;
 ...
 *
 */
public class SkinMarket extends ShopScervice<SkinShopCfg> {

	private static final SkinMarket instance = new SkinMarket();

	public static SkinMarket getInstance() {
		return instance;
	}
	 ...
	 * @param luckyScore
	 */
	public List<SkinShopCfg> shopItemInfo(Player player, SkinShopEntity entity,boolean isCreate) {

		entity.getShopMap().clear();
		// vip等级对应物品数量
		//int showItemCount = this.getMaxShopItemCount(player.getVipLevel());
		// 等级对应物品集
		List<SkinShopCfg> ShopList = SkinShopCfg.getShopCfgs();
		// 商品集合clone
		List<SkinShopCfg> ShopCfgsClone = new ArrayList<SkinShopCfg>(ShopList);

		// 商城显示集合
		//List<SkinShopCfg> crystalItemList = this.getItemList(DailyShopCfgsClone, showItemCount);

		for (int i = 0; i < ShopCfgsClone.size(); i++) {
			DailyShopItem dailyShopItem = SkinShopCfg.getDailyShopInfo(ShopCfgsClone.get(i), i + 1);
	...
	public void sendShopBuilder(Player player) {
		...
		SkinShopEntity entity = player.getPlayerData().loadSkinShopEntity();
...
//	}
	...
	public void shopBuyLogic(Player player, BuyShopItemsRequest request) {
		...
		SkinShopEntity ShopEntity = player.getPlayerData().loadSkinShopEntity();
	...
	public ShopType getShopType() {
		return Const.ShopType.SKIN_MARKET;
	}
