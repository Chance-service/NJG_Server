 ...
 *
 */
public class ArenaMarket extends ShopScervice<ArenaShopCfg> {

	private static final ArenaMarket instance = new ArenaMarket();

	public static ArenaMarket getInstance() {
		return instance;
	}
	 ...
	 * @param luckyScore
	 */
	public List<ArenaShopCfg> shopItemInfo(Player player, ArenaShopEntity entity,boolean isCreate) {

		entity.getShopMap().clear();
		// vip等级对应物品数量
		//int showItemCount = this.getMaxShopItemCount(player.getVipLevel());
		// 等级对应物品集
		//List<ArenaShopCfg> ShopList = ArenaShopCfg.getShopCfgs();
		// 商品集合clone
		//List<ArenaShopCfg> ShopCfgsClone = new ArrayList<ArenaShopCfg>(ShopList);

		// 商城显示集合
		List<ArenaShopCfg> ItemList = new ArrayList<>();
		ItemList = this.getItemList(player.getLevel());

		for (int i = 0; i < ItemList.size(); i++) {
			DailyShopItem dailyShopItem = ArenaShopCfg.getShopInfo(ItemList.get(i), i + 1);
	...
	public void sendShopBuilder(Player player) {
		...
		ArenaShopEntity entity = player.getPlayerData().loadArenaShopEntity();

		// 商城构建
		Map<Integer, DailyShopItem> ItemMap = entity.getShopMap();
		...
		// 商城物品构建
		for (DailyShopItem item : ItemMap.values()) {
			...
			ArenaShopCfg acfg = ArenaShopCfg.getCfgbyIndex(item.getCfgIndex());
			builder.addData(this.createDisplayData(DataType.ITEM_LEFT,item.getLeft()));
			builder.addData(this.createDisplayData(DataType.LEVEL_LIMIT,acfg.getMinLv()));
		}
...
//	}
	...
	public void shopBuyLogic(Player player, BuyShopItemsRequest request) {
		...
		ArenaShopEntity ShopEntity = player.getPlayerData().loadArenaShopEntity();
		...
		ArenaShopCfg shopcfg = ArenaShopCfg.getCfgbyIndex(shopItem.getCfgIndex());
		...
		if (player.getLevel() < shopcfg.getMinLv()) {
			player.sendError(HP.code.SHOP_BUY_C_VALUE, Status.error.LEVEL_NOT_LIMIT);
			return;
		}
		...
		awardItems.rewardTakeAffectAndPush(player, Action.SHOP_BUY_TOOLS, 1,TapDBSource.Arena_Shop,Params.valueOf("id",shopcfg.getId()));
	...
	public List<ArenaShopCfg> getItemList(int lv) {
		List<ArenaShopCfg> ShopCfgs = new ArrayList<ArenaShopCfg>();
		if (SysBasicCfg.getInstance().getArenaItemList().size() != ArenaShopCfg.getGroupbyCfg().size()) {
			throw new RuntimeException("SysBasicCfg's ArenaItemList size not match ArenaShopCfg's random gourp");
			//return ShopCfgs;
		}
		int group = 0;
		List<ArenaShopCfg> goupCfgList = new ArrayList<>();
		List<Integer> weightList = new ArrayList<>();
		for (Integer randNumber :SysBasicCfg.getInstance().getArenaItemList()) {
			goupCfgList.clear();
			weightList.clear();
			for (ArenaShopCfg Acfg : ArenaShopCfg.getGroupbyCfg().get(group)) {
				if (( lv >= Acfg.getMinLv()) && (lv <= Acfg.getMaxlv())) {
					goupCfgList.add(Acfg);
					weightList.add(Acfg.getWeight());
				}
			}
			if (goupCfgList.size() >= randNumber) {
				ShopCfgs.addAll(GuaJiRand.randonWeightObject(goupCfgList,weightList, randNumber));
			}
			group++;
		}
		return ShopCfgs;
	...
	public ShopType getShopType() {
		return Const.ShopType.ARENA_MARKET;
	}
