...
import com.guaji.game.config.GuildShopCfg;
 ...
 *
 */
public class GuildMarket extends ShopScervice<GuildShopCfg> {

	private static final GuildMarket instance = new GuildMarket();

	public static GuildMarket getInstance() {
		return instance;
	}
	 ...
	 * @param luckyScore
	 */
	public List<GuildShopCfg> shopItemInfo(Player player, GuildShopEntity entity,boolean isCreate) {

		entity.getShopMap().clear();
		// vip等级对应物品数量
		//int showItemCount = this.getMaxShopItemCount(player.getVipLevel());
		// 等级对应物品集
		List<GuildShopCfg> ShopList = GuildShopCfg.getShopCfgs();
		// 商品集合clone
		List<GuildShopCfg> ShopCfgsClone = new ArrayList<GuildShopCfg>(ShopList);

		// 商城显示集合
		//List<GuildShopCfg> crystalItemList = this.getItemList(DailyShopCfgsClone, showItemCount);

		for (int i = 0; i < ShopCfgsClone.size(); i++) {
			DailyShopItem ShopItem = GuildShopCfg.getShopInfo(ShopCfgsClone.get(i), i + 1);
...
//		}
		...
		GuildShopEntity entity = player.getPlayerData().loadGuildShopEntity();
...
//	}
	...
	public void shopBuyLogic(Player player, BuyShopItemsRequest request) {
		...
		GuildShopEntity ShopEntity = player.getPlayerData().loadGuildShopEntity();
	...
	public ShopType getShopType() {
		return Const.ShopType.GUILD_MARKET;
	}
