...
public class RechargeConfig {
	...
	public RechargeItem getByName(String strName) {

		for (Integer key : rechargeCfgMap.keySet()) {
			RechargeItem item = rechargeCfgMap.get(key);
		...
		try {
			if (platform == null || "".equals(platform)) {
				if (defaultRechargeConfig == null) {
					String filePath = "conf/rechargeConfig/rechargeConfig.xml";
			...
			String filePath = "conf/rechargeConfig/rechargeConfig_" + platform + ".xml";
	...
	private static RechargeConfig loadRechargeConfig(File file) {
		try {
			...
			for (int i = 0; i < rechargeItems.getLength(); i++) {
				...
				rechargeItem.setShowAddNum(Integer.valueOf(attrMap.getNamedItem("showAddNum") == null ? "0"
						: attrMap.getNamedItem("showAddNum").getNodeValue()));
				...
				if (attrMap.getNamedItem("productType") != null) {
					rechargeItem.setProductType(attrMap.getNamedItem("productType").getNodeValue());
				}
